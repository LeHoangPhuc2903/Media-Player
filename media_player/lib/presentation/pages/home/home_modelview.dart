...
import '../../../domain/use_cases/playlist/playlist_controller.dart';
  ...
  Future<void> scanAndLoadData() async {
    try {
      ...
      for (var playlist in updatedPlaylists) {
        playlist.audioFiles = audioFiles.where((audio) => playlist.audioIds.contains(audio.id)).toList();
...
*/
  Future<Playlist?> getPlaylistByIdWithAudios(String id) async {
    try {
      ...
      final matchingAudios = audioFiles.where((audio) => playlist.audioIds.contains(audio.id)).toList();
